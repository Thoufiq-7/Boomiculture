...
export default function AdvancedNavbar() {
    ...
    return (
        <>
          ...
          ${scrolled
                        ? "bg-green-950/80 shadow-2xl border-b border-green-700"
                        : "bg-green-900/40"
                    }`}
            >
                <nav className="max-w-7xl mx-auto px-6 sm:px-12 md:px-20 lg:px-32">
                    <div className="flex items-center justify-between h-16">
                        {/* Logo with animated color glow */}
                        <Link
                            to="/"
                            className={`text-3xl font-black tracking-widest cursor-pointer select-none
                transition-colors duration-700 ${scrolled
                                    ? "text-green-300 drop-shadow-[0_0_12px_rgba(72,187,120,0.8)]"
                                    : "text-green-200 drop-shadow-[0_0_6px_rgba(72,187,120,0.6)]"
                        ...
                        <div className="hidden md:flex space-x-14 font-semibold text-lg">
                            {navItems.map(({ label, path }) => (
                                <Link
                                    key={path}
                                    to={path}
                                    className={`relative px-4 py-2 rounded-lg transition-colors duration-300 ease-in-out hover:text-green-400 focus:outline-none focus:ring-2 focus:ring-green-400
                    ${location.pathname === path
                                            ? "text-green-400 font-extrabold shadow-[0_0_10px_rgba(72,187,120,0.9)]"
                                            : "text-white"
                                        }`}
                                >
                                    {label}
